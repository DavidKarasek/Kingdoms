                ...
                if (parent.getParent().getKingdomsManager().getVillage(name) == null) {
                    ...
                    if (!name.matches("[!-@\\[-`](.*)")) {
                        if (parent.getParent().getConfiguration().isEconomy()) {
                            if (parent.getParent().getEconomy().withdraw((OfflinePlayer) player,
                                    parent.getParent().getConfiguration().getVillageCost()) == 0) {
                                player.sendMessage(Kingdoms.PREFIX + "Successfully created village "
                                        + name);
                                Village village = new Village(name, player.getLocation().getWorld());
                                village.setBalance(parent.getParent().getConfiguration().getDefaultBalance());
                                ...
                                parent.getParent().getKingdomsManager().addVillageBlock(newBlock);
                                parent.getParent().getMySqlConnector().addVillage(parent.getParent().getUuidHolder()
                                        .getUuid(player.getName()), village, newBlock);
                                parent.getParent().getKingdomsManager().getPlayer(player).setVillage(village);
                                if (parent.getParent().getConfiguration().getUpkeep() != 0 && parent.getParent().getEconomy() != null) {
                                ...
                                player.sendMessage(Kingdoms.ERROR_PREFIX + "You don't have enough " +
                                        ...
                                        + parent.getParent().getConfiguration().getVillageCost());
                            }
                        } else {
